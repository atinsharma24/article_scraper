import type { Article, ArticleListResponse, ArticleDetailResponse, ListArticlesOptions } from './types';
...
export async function listArticles({ type, parentId, perPage = 10 }: ListArticlesOptions = {}): Promise<ArticleListResponse> {
  const apiBaseUrl = getApiBaseUrl();
...
export async function getArticle(id: number): Promise<ArticleDetailResponse | null> {
  return requestJson<ArticleDetailResponse>(`/api/articles/${id}`);
}
